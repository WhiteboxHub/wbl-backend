...
class TestStorageAndChunks(unittest.TestCase):
    ...
    def test_save_chunk_success(self):
        ...
        self.assertTrue(path.endswith("0.webm"))
    ...
    def test_save_chunk_idempotency_on_retry(self):
        ...
        self.assertEqual(size2, len(content2))
    ...
    def test_chunk_status_detection(self):
        ...
        self.assertEqual(status["missing_chunks"], [1])
        self.assertFalse(status["is_ready_for_assembly"])
    ...
    def test_assemble_chunks_success(self):
        ...
        self.assertTrue(os.path.exists(assembled_path))
        with open(assembled_path, "rb") as f:
            data = f.read()

        self.assertEqual(data, b"Part0_Part1_Part2")
    ...
    def test_assemble_chunks_missing_raises_missing_chunks_error(self):
        ...
        with self.assertRaises(MissingChunksError) as cm:
            storage_service.assemble_chunks(self.candidate_id, self.assessment_id, total_chunks=2)

        self.assertEqual(cm.exception.missing_chunks, [1])

    @patch("subprocess.run")
    def test_extract_audio_generates_wav(self, mock_subproc):
        ...
        self.assertTrue(audio_path.endswith("audio.wav"))
    ...
    def test_gdpr_candidate_media_purge(self):
        ...
        self.assertTrue(os.path.exists(storage_service.get_chunks_dir(self.candidate_id, self.assessment_id)))

        purged = storage_service.purge_candidate_data_gdpr(self.candidate_id)
        self.assertTrue(purged)
        self.assertFalse(os.path.exists(os.path.join(storage_service.base_path, str(self.candidate_id))))
