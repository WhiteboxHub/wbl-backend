...
def fetch_keyword_presentation(search: str, course: str):
    ...
    if not type_codes:
        raise HTTPException(
            ...
            detail="Invalid search keyword. Please select one of: Presentations, Cheatsheets, Study Guides, Interactive Visual Explainers, Templates, Books, Newsletters, Questions, Git Repo's"
        )
...
# fetch_subject_batch_recording is defined below (with search + cache support)
...
# fetch_course_batches is defined below (with course param support)
...
def fetch_subject_batch_recording(
    ...
    return {"batch_recordings": [r[0] for r in recordings]}
