...
from fapi.schemas import LeadORM,CandidateORM
from datetime import datetime
from fapi.models import CandidateNewORM as CandidateNewORM, Base
...
def get_lead_info_mark_move_to_candidate_true(lead_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if lead.moved_to_candidate:
            logger.warning(f"⚠️ Lead already moved to candidate: {lead.id}")
...
def move_lead_to_candidate(lead_id: int, db: Session):
    ...
    try:
        ...
        candidate_data = {
            ...
            'status': 'active',
            'notes': lead.notes,
            'batchid': 1,  # IMPORTANT: Set this properly in your real code
        ...
        new_candidate = CandidateNewORM(**candidate_data)
        db.add(new_candidate)
        
        # Step 5: Verify pre-commit state
