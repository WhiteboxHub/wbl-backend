...
from fapi.models import VendorContactExtractCreate,DailyVendorActivity,DailyVendorActivityCreate  # ✅ Make sure this is imported
...
async def insert_vendor(data: Dict) -> Dict:
    try:
        ...
        cursor.execute("SELECT * FROM vendor WHERE id = %s", (inserted_id,))
        result = cursor.fetchone()
...
def delete_vendor_by_id_sync(vendor_id: int) -> bool:
    conn = None
    try:
        conn = get_connection()
        cursor = conn.cursor()
        ...
        print(" Error in delete_vendor_by_id_sync:", str(e))
        return False
    finally:
        if conn:
            conn.close()
...
async def get_all_daily_vendor_activities():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        query = "SELECT * FROM vendor_daily_activity ORDER BY activity_id DESC"
        await loop.run_in_executor(None, cursor.execute, query)
        rows = cursor.fetchall()
        return rows
    except Error as e:
...
async def insert_daily_vendor_activity(activity: DailyVendorActivityCreate):
    ...
    try:
        ...
        query = """
            INSERT INTO vendor_daily_activity (
                vendor_id, application_date, linkedin_connected, 
                contacted_on_linkedin, notes, employee_id
            )
            VALUES (%s, %s, %s, %s, %s, %s)
        """
...
async def update_daily_vendor_activity(activity_id: int, fields: dict):
    if not fields:
        raise HTTPException(status_code=400, detail="No data to update")
    ...
    try:
        ...
        if 'vendor_id' in fields and fields['vendor_id'] is not None:
            check_query = "SELECT id FROM vendor WHERE id = %s"
            await loop.run_in_executor(None, cursor.execute, check_query, (fields['vendor_id'],))
            result = cursor.fetchone()
            if result is None:
                raise HTTPException(status_code=400, detail=f"Vendor ID {fields['vendor_id']} does not exist")
        set_clause = ", ".join([f"{key} = %s" for key in fields.keys()])
        values = list(fields.values())
        ...
        if cursor.rowcount == 0:
            raise HTTPException(status_code=404, detail="Daily vendor activity not found")
...
async def delete_daily_vendor_activity(activity_id: int):
    loop = asyncio.get_event_loop()
    conn = await loop.run_in_executor(None, lambda: mysql.connector.connect(**db_config))
