...
def check_linkedin_contacts_version(db: Session = Depends(get_db)):
    try:
        result = db.query(
            ...
            func.max(LinkedinOnlyContact.id).label("max_id"),
            func.sum(
                func.crc32(
                    func.concat_ws(
                        '|',
                        LinkedinOnlyContact.id,
                        func.coalesce(LinkedinOnlyContact.name, ''),
                        func.coalesce(LinkedinOnlyContact.linkedin_id, ''),
                        func.coalesce(LinkedinOnlyContact.job_title, '')
                    )
                )
            ).label("checksum")
        ).first()
