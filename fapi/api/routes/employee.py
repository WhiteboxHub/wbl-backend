...
def employee_candidates(employee_id: int, db: Session = Depends(get_db)):
    emp = db.query(models.EmployeeORM).filter(
        models.EmployeeORM.id == employee_id
    ).first()

    if not emp:
        raise HTTPException(status_code=404, detail="Employee not found")

    return get_employee_candidates(db, employee_id)


@router.get("/employees/{employee_id}/session-class-data")
def get_employee_session_class_data(
    employee_id: int,
