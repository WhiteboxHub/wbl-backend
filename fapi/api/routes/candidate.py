...
from fapi.db.models import CandidateInterviewNew,CandidateStatus
...
def list_interviews(
    ...
    return (
        db.query(CandidateInterviewNew)
        .order_by(CandidateInterviewNew.interview_date.desc())
        .options(joinedload(CandidateInterviewNew.candidate)) 
        .offset(skip)
        .limit(limit)
        .all()
    )
