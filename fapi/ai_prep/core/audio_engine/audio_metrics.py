...
def load_audio_waveform(audio_input: Union[str, np.ndarray], sr: int = AUDIO_CONFIG.SAMPLE_RATE) -> np.ndarray:
    """
    Loads audio waveform from any audio or video container (.wav, .mp3, .webm, .mp4, .mov, etc.)
    using soundfile with an automatic PyAV / FFmpeg demuxer fallback.
    """
    if isinstance(audio_input, np.ndarray):
        return audio_input.astype(np.float32)

    # 1. Try standard audio decoding via soundfile
    try:
    ...
    # 3. Last-resort fallback to librosa
    try:
...
def calculate_audio_metrics(
    ...
    Supports both audio files (.wav, .mp3) and video files (.mp4, .webm).
    """
    y = load_audio_waveform(audio_input, sr=sr)
