...
class BaseTranscriptionProvider(ABC):
    ...
    {
        "transcript_text": str,
        "word_timestamps": List[Dict[str, Any]],  # [{"word": str, "start": float, "end": float, "probability": float}]
        "duration": float
    }
    """
    ...
    def transcribe(
        ...
        **kwargs: Any
        ...
        logger.warning(
            ...
            "To enable live socket STT, connect your WebSocket stream client here."
        )

        # Integration Point: When the live socket server is ready, implement the streaming client here.
        # Fallback to empty normalized structure if no precomputed socket stream is available:
        return {
            "transcript_text": "",
            "word_timestamps": [],
            "duration": 0.0
...
class BrowserTranscriptionProvider(BaseTranscriptionProvider):
    ...
    def transcribe(
        ...
        **kwargs: Any
        ...
        if not precomputed_transcript_text:
...
def get_transcription_provider(provider_name: Optional[str] = None) -> BaseTranscriptionProvider:
    """
    Factory to instantiate the configured transcription provider.
    ...
    logger.info(f"Initialized Transcription Provider: '{selected}'")
    return provider_cls()
